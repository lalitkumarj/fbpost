...
class Post(ndb.Model):
    page_post_url = ndb.StringProperty(required=True)
...
#TODO: You should only get one page!
        ...
        for group_id in select_groups:
            posted_id =  self.graph.put_photo(blob_reader, description, album_id=group_id)['id']
            post = Post(parent=ndb.Key("User", self.current_user['id']),
                        post_url=posted_id, 
                        page_post_url = page_posted_id,
                        picture_url=images.get_serving_url(blob_key),
                        container = page_names[page_id],
                        status = "active") 
            post.put()
...
#need a list of posts. get this list of posts by sorting by page_id. a post should be a dictionary . has a group field that has a list of 
            all_posts = [dict([('post_url',"http://facebook.com/"+q.post_url), 
                           ('date', q.date), 
                           ('container', q.container),
                           ('picture_url', q.picture_url)]) for q in query.iter()]
...
application = webapp2.WSGIApplication(
    [('/', HomeHandler), ('/login', LoginHandler), ('/portal', PortalHandler) , ('/manage', ManageHandler), ('/upload',UploadHandler), ('/service', ServiceHandler),('/service/post',ServiceHandler), ('/redirect?',RedirectHandler), ('/logout', LogoutHandler), ('/end',EndHandler)],
    debug=True,
    config=config
)
